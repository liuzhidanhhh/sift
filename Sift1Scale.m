...
absolute_sigma = zeros(octave, s + 3);
...
%    pause;
%    close(fig)
   figure(1);clf; imagesc(pic); colormap gray; axis equal image off;
...
%    pause;
%    close(fig)
   figure(2);clf; imagesc(pic); colormap gray; axis equal image off;
   cdata = print('-RGBImage');
...
octave = o;
for interval = 2 : ( s + 1)
    g = gaussian_filter(1.5 * absolute_sigma(octave, interval));
    Ifilters{octave, interval} = g(:) * g(:)';
    Hfilters(octave, interval) = floor(length(g) / 2.0);
end

% Enumerate the found key points
numKey = size(keypoints, 1);
keypoint_count = 0;
for i = 1 : numKey
    ...
    odiff = repmat(ori(:),[1 num_bins]) - repmat(hist_orient,[numel(ori), 1]);
    odiff = mod(odiff, 2 * pi); 
    odiff = odiff - pi; % the difference should be in [-pi, pi)
...
if(interactive >= 1)
    ...
    %hold on;
    %quiver(pos(:,1),pos(:,2), cos(orient),sin(orient),0.5,'linewidth',3)
    cdata = print('-RGBImage');
